  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
        ...
        {/* Collection Risk (Top overdue) */}
        {Array.isArray(stats.topOverdueOrgs) && stats.topOverdueOrgs.length > 0 && (
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-lg font-bold text-gray-900">Collection Risk</h2>
                <p className="text-sm text-gray-500">Top overdue buyers by outstanding</p>
              </div>
            </div>
            <div className="space-y-3">
              {stats.topOverdueOrgs.map((o) => (
                <div key={o.orgId} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-100">
                  <div className="min-w-0">
                    <div className="font-semibold text-gray-900 text-sm truncate">{o.orgName}</div>
                    <div className="text-xs text-gray-500">{o.invoiceCount} overdue invoices</div>
                  </div>
                  <div className="text-right">
                    <div className="text-base font-bold text-red-700">
                      ₹{Math.round(o.overdueOutstanding || 0).toLocaleString("en-IN")}
                    </div>
                    <div className="text-xs text-gray-500">Overdue outstanding</div>
                  </div>
                </div>
              ))}
            </div>
